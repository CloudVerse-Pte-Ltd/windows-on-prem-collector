...
| Telemetry misattribution | SCVMM performance uses per-VM `Get-SCPerformanceData` keyed by VMM GUID. Local Hyper-V counters are used only in standalone mode. Missing/invalid/static-memory evidence produces a typed gap and never a synthetic zero. |

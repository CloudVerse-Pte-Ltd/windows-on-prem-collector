...
foreach ($required in @("PSObject.Properties.Name -contains 'offlineExportDirectory'", "if ([string]`$configuration.mode -eq 'SCVMM') { 'Password' } else { 'ServiceAccount' }")) { if (-not $source.Contains($required)) { throw "Installer omits optional-config or gMSA task-logon invariant: $required" } }
foreach ($required in @('SeBatchLogonRight','secedit.exe','USER_RIGHTS','Failed to grant the collector batch-logon right')) { if (-not $source.Contains($required)) { throw "Installer omits collector batch-logon grant: $required" } }
foreach ($required in @('CloudVerseDataCenterCollectorValidation','upgrade-validation.stdout.log','upgrade-validation.stderr.log','dist/src/runtime/cli.js validate collector.config.json')) { if (-not $source.Contains($required)) { throw "Installer omits fixed validation-task invariant: $required" } }
foreach ($required in @('Collector service-identity validation timed out','Collector service identity could not validate the configured execution boundary','Collector service-identity validation returned malformed output','Collector service-identity validation did not match the installed configuration','Get-ScheduledTaskInfo','Start-ScheduledTask -TaskName ''CloudVerseDataCenterCollectorValidation''','Start-ScheduledTask -TaskName ''CloudVerseDataCenterCollector'' -ErrorAction Stop')) { if (-not $source.Contains($required)) { throw "Installer omits fail-closed service-identity preflight invariant: $required" } }

...
export interface WindowsCollectorConfig { stateDirectory: string; spoolDirectory: string; scriptsDirectory: string; manifestPath: string; managementPlaneUid: string; mode: 'SCVMM' | 'LOCAL_HYPERV'; scvmm?: { server: string; port: number }; upload?: { endpoint: string; allowedHosts: string[]; privateAddressAllowedHosts?: string[] }; intervalSeconds: number; maxSpoolBytes: number; maxSpoolItems: number; offlineExportDirectory?: string }
export async function loadWindowsCollectorConfig(path: string): Promise<WindowsCollectorConfig> { const value = JSON.parse(await readFile(path, 'utf8')) as WindowsCollectorConfig; if (!['SCVMM', 'LOCAL_HYPERV'].includes(value.mode) || !Number.isSafeInteger(value.intervalSeconds) || value.intervalSeconds < 60) throw new Error('Collector mode or interval is invalid'); if (value.mode === 'SCVMM' && !value.scvmm) throw new Error('SCVMM configuration is required'); if (!value.upload && !value.offlineExportDirectory) throw new Error('Upload or offline export must be configured'); return value }
...
export async function collectPerformanceForMode(mode: WindowsCollectorConfig['mode'], runner: ConstrainedPowerShellRunner): Promise<{ facts: HypervMetricFact[]; gaps: HypervMetricGap[] }> {
  ...
  // Local Hyper-V counter instances describe only the machine executing the
  // collector. An SCVMM management server is not an estate host, so attaching
  // those counters to SCVMM inventory would silently misattribute telemetry.
  // Publish an explicit retryable gap until the signed SCVMM performance
  // operation is enabled and verified against a real VMM estate.
  return { facts: [], gaps: [{ code: 'SCVMM_PERFORMANCE_ADAPTER_REQUIRED', metricKey: 'hyperv.vm.performance', details: { transport: 'SCVMM_POWERSHELL', localCountersRejected: true } }] }
}

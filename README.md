...
2. Have the enterprise-approved code-signing pipeline Authenticode-sign all operational `.ps1` files, both installers, and every executable packaged JEA `.ps1`, `.psm1`, and `.psd1` asset with the same certificate. PowerShell does not support Authenticode signing for the declarative `.psrc` role-capability format; its exact bytes remain integrity-bound by the mandatory whole-package SHA-256. Then run `New-CloudVerseReleaseManifest.ps1` to generate `release-manifest.json` with exact post-signing operation hashes and signer thumbprints. Repack the candidate contents at the ZIP root under the final name `cloudverse-windows-collector.zip`; do not add another parent directory.
3. Select `executionBoundary.kind` in the collector configuration. For `JEA`, give an endpoint name; the signed installer creates the supplied `NoLanguage` endpoint, verifies every executable JEA asset against its own signer, and requires the package-digest-bound `.psrc` file. For `WDAC_APPLOCKER`, deploy policy that makes direct Windows PowerShell report `ConstrainedLanguage`.
...
Connected mode optionally supports an authenticated HTTPS forward proxy. Both the CPD origin and proxy use separate exact-host allowlists and DNS/private-address policy. Proxy authorization is read from an ACL-protected file inside `stateDirectory` (a single `Basic …` or `Bearer …` header value); credentials in the proxy URL or collector configuration are rejected. Proxy failure defers encrypted spool items using the same bounded retry path.

Upgrade compatibility is explicit: this release produces bundle schema `1.0` and can deliver queued current (`1.0`) and N-1 (`0.9`) bundles byte-for-byte. Older or unknown schemas remain encrypted in place and appear as incompatible in health; they are never deleted, rewritten, or mislabelled. Operators can therefore roll back to a compatible worker or export the preserved item for support recovery.

`scripts/Update-CloudVerseCollector.ps1` is the signed in-place upgrade entry point. It verifies the archive digest and requires every executable PowerShell asset to share the upgrade script's valid Authenticode signer before stopping the scheduled task. It preserves the installed configuration and external state/spool directories, swaps only the immutable code tree, re-establishes the signed JEA endpoint when selected, and restarts the existing task. Any failure restores the previous code tree and JEA endpoint; a successful upgrade retains a uniquely named rollback tree and reports its exact path for operator-controlled removal after acceptance.

...
describe('C24 Windows collector security boundary', () => {
  ...
  it('the trusted JEA functions load every fixed host-native read-only command module without exposing Import-Module', async () => {
    ...
    for (const fixedImport of ['Import-Module CimCmdlets -ErrorAction Stop -DisableNameChecking', 'Import-Module FailoverClusters -ErrorAction Stop -DisableNameChecking', 'Import-Module Hyper-V -ErrorAction Stop -DisableNameChecking', 'Import-Module Microsoft.PowerShell.Diagnostics -ErrorAction Stop -DisableNameChecking']) expect(module).toContain(fixedImport)
    for (const preference of ["$WarningPreference = 'SilentlyContinue'", "$ProgressPreference = 'SilentlyContinue'", "$InformationPreference = 'SilentlyContinue'"]) expect(module).toContain(preference)
  ...
  it('the C32 source uses only local read-only counters and emits explicit mapping gaps', async () => {
    ...
    for (const command of ['Get-VM', 'Get-VMHardDiskDrive', 'Get-Counter', 'ConvertTo-Json']) expect(source).toContain(command)

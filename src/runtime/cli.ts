...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (command === 'run' || command === 'once') { const runtime = await createWindowsCollectorRuntime(await loadWindowsCollectorConfig(argv[1] ?? required('COLLECTOR_CONFIG_FILE'))); if (command === 'once') { const result = await runtime.collectOnce(); await runtime.flush(); process.stdout.write(`${JSON.stringify(result)}\n`) } else { const controller = new AbortController(); process.once('SIGINT', () => controller.abort()); process.once('SIGTERM', () => controller.abort()); await runtime.run(controller.signal) }; return }
  throw new Error('Usage: cloudverse-windows-collector <enroll|once|run> [config.json]')
}

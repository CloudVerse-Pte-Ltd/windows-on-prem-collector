...
| Runtime replacement/persistence | The service identity receives read/execute only on the immutable install tree, including `node.exe`, compiled JavaScript, configuration and signed scripts. Writable state, encrypted spool and optional offline export must be absolute, non-root paths outside that tree and receive separate `Modify` ACLs. |
| Upgrade substitution or partial write | The signed upgrade entry point verifies the exact archive digest and common Authenticode signer before stopping collection. It stages beside the bounded install directory, preserves configuration/state/spool, swaps the immutable tree, re-creates JEA, and rolls code/JEA back automatically on failure. The prior tree is retained after success until an operator explicitly retires it. |
| PowerShell policy bypass | Machine WDAC/AppLocker or JEA endpoint is mandatory and selected explicitly. `AllSigned` is defense in depth, not the sole boundary. Direct execution requires `ConstrainedLanguage`; JEA requires `NoLanguage`, exposes only the four versioned catalog functions plus a fixed boundary-health probe, and records ACL-protected transcripts. The installer verifies every executable JEA `.ps1`/`.psm1`/`.psd1` asset shares its approved signer; the declarative `.psrc` role capability is bound by the mandatory whole-package SHA-256 because that format cannot be Authenticode-signed. |
